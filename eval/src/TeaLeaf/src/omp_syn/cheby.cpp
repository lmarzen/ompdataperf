...
void cheby_calc_u(const int x, const int y, const int halo_depth, double *u, const double *p) {
  ...
  #pragma omp target update from(u[ : n], p[ : n])
  #pragma omp target update to(u[ : n], p[ : n])
...
#ifdef OMP_TARGET
  int n = (y - halo_depth) * (x - halo_depth);
  #pragma omp target update from(w[ : n], p[ : n], r[ : n], u0[ : n])
  #pragma omp target update to(w[ : n], p[ : n], r[ : n], u0[ : n])
  #pragma omp target teams distribute parallel for simd collapse(2)
#else
  #pragma omp parallel for
#endif
