...
void jacobi_init(const int x, const int y, const int halo_depth, const int coefficient, double rx, double ry, const double *density,
  ...
  #pragma omp target update from(energy[ : n], density[ : n], u[ : n], u0[ : n])
  #pragma omp target update to(energy[ : n], density[ : n], u[ : n], u0[ : n])
...
#ifdef OMP_TARGET
  #pragma omp target update from(density[ : n], kx[ : n], ky[ : n])
  #pragma omp target update to(density[ : n], kx[ : n], ky[ : n])
  #pragma omp target teams distribute parallel for simd collapse(2)
#else
  #pragma omp parallel for
#endif
...
#ifdef OMP_TARGET
  int n = (y - halo_depth) * (x - halo_depth);
  #pragma omp target update from(u[ : n], r[ : n])
  #pragma omp target update to(u[ : n], r[ : n])
  #pragma omp target teams distribute parallel for simd collapse(2)
#else
  #pragma omp parallel for
#endif
...
#ifdef OMP_TARGET
  #pragma omp target update from(r[ : n], kx[ : n], ky[ : n])
  #pragma omp target update to(r[ : n], kx[ : n], ky[ : n])
