...
void field_summary(const int x, const int y, const int halo_depth, const double *volume, const double *density, const double *energy0,
                   const double *u, double *volOut, double *massOut, double *ieOut, double *tempOut) {
  double vol = 0.0;
  double ie = 0.0;
  double temp = 0.0;
  double mass = 0.0;
...
#ifdef OMP_TARGET
  int n = (y - halo_depth) * (x - halo_depth);
  #pragma omp target update from(u[ : n], u0[ : n])
  #pragma omp target update to(u[ : n], u0[ : n])
...
#ifdef OMP_TARGET
  int n = (y - halo_depth) * (x - halo_depth);
  #pragma omp target update from(r[ : n], u0[ : n])
  #pragma omp target update to(r[ : n], u0[ : n])
  #pragma omp target teams distribute parallel for simd collapse(2)
#else
  #pragma omp parallel for
#endif
...
#ifdef OMP_TARGET
  int n = (y - halo_depth) * (x - halo_depth);
  #pragma omp target update from(buffer[ : n])
  #pragma omp target update to(buffer[ : n])
...
void finalise(const int x, const int y, const int halo_depth, double *energy, const double *density, double *u) {
#ifdef OMP_TARGET
  int n = (y - halo_depth) * (x - halo_depth);
  #pragma omp target update from(density[ : n], energy[ : n], u[ : n])
  #pragma omp target update to(density[ : n], energy[ : n], u[ : n])
  #pragma omp target teams distribute parallel for simd collapse(2)
#else
  #pragma omp parallel for
#endif
