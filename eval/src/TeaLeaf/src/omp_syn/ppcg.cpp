...
void ppcg_init(const int x, const int y, const int halo_depth, double theta, const double *r, double *sd) {
  ...
  #pragma omp target update from(sd[ : n], r[ : n])
  #pragma omp target update to(sd[ : n], r[ : n])
...
#ifdef OMP_TARGET
  int n = (y - halo_depth) * (x - halo_depth);
  #pragma omp target update from(sd[ : n], r[ : n], u[ : n])
  #pragma omp target update to(sd[ : n], r[ : n], u[ : n])
  #pragma omp target teams distribute parallel for simd collapse(2)
#else
  #pragma omp parallel for
#endif
