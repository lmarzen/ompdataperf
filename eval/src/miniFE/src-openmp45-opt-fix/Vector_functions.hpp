...
void
  dot(typename TypeTraits<typename Vector::ScalarType>::magnitude_type* result,
      const Vector& x,
      const Vector& y)
{
  ...
  *result = 0;
  ...
  #pragma omp target teams distribute parallel for reduction(+:result[0]) \
	num_teams(512) firstprivate(n)
  for(int i=0; i<n; ++i) {
    result[0] += xcoefs[i] * ycoefs[i];
  }
...
template<typename Vector>
void
  dot_r2(typename TypeTraits<typename Vector::ScalarType>::magnitude_type* result, const Vector& x)
{
#ifdef MINIFE_DEBUG_OPENMP
 	int myrank;
