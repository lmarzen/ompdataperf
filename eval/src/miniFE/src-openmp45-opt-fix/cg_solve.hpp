...
//  |inner| < 100 * ||v||_2 * ||w||_2 * epsilon
  ...
  magnitude norm;
#pragma omp target enter data map(alloc:norm)
  dot(&norm,v,v);
  magnitude vnorm = std::sqrt(norm);
  dot(&norm,w,w);
  magnitude wnorm = std::sqrt(norm);
#pragma omp target exit data map(release:norm)
...
//  print_vec(p.coefs, "p");
  ...
  TICK(); dot_r2(&rtrans, r); TOCK(tDOT);
...
//std::cout << "rtrans="<<rtrans<<std::endl;
  ...
  for(LocalOrdinalType k=1; k <= max_iter && normr > tolerance; ++k) {
    ...
    TICK(); dot(&p_ap_dot, Ap, p); TOCK(tDOT);
  ...
  #pragma omp target exit data           \
       	map(from:  x_ptr[:x.coefs.size()])  \
        map(release:  b_ptr[:b.coefs.size()],  \
	              r_ptr[:r.coefs.size()],  \
                      p_ptr[:p.coefs.size()],  \
               	      Ap_ptr[:Ap.coefs.size()], \
                      Arowoffsets[:A.row_offsets.size()], \
